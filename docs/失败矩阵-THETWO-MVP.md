...
AI companion + AR 产品的失败路径很多。本文档用统一矩阵固化主链路的失败处理，避免后续真实 LLM、EasyAR、截图和数据删除接入后出现不可预期行为。
...
| EasyAR | SDK 初始化失败 | 初始化返回失败或异常 | 进入 CameraX 相机叠加 | 提示当前设备进入兼容召唤 | 否 |
| EasyAR | 图像跟踪不可用 | 能力检测失败 | 进入 CameraX 相机叠加 | 提示当前设备不支持锚点召唤 | 否 |
| EasyAR | 锚点图识别失败 | 超时未识别官方锚点图 | 保持扫描或允许相机叠加 | 提示重新对准官方锚点图 | 否 |
| EasyAR | 平面识别不可用 | 平面检测能力缺失或传感器不足 | 保持锚点图平面召唤 | 提示已使用锚点召唤模式 | 否 |
| EasyAR | 丢锚超过阈值 | target tracking lost | 保持短时显示，超时提示重扫 | 提示重新对准锚点图 | 否 |
...
- EasyAR unavailable -> fallback camera or screen-only。
- plane detection unavailable -> keep marker-plane summon。
...
- 不允许 EasyAR 初始化失败阻断 fallback 召唤。
