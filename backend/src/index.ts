...
function logDerivedEvent(input: {
  event: "chat.summary.refresh" | "chat.memory.refresh" | "chat.safety_state.updated";
  userId: string;
  outcome: string;
  mode?: string;
  durationMs?: number;
...
app.get("/health/config", (_req, res) => {
  res.json(
    ok({
      ...
      llmBaseUrlConfigured: !!config.llmBaseUrl,
      llmModelConfigured: !!config.llmModel,
...
app.post("/auth/request-code", async (req, res) => {
  ...
  res.json(
    ok(
      response,
      sendResult.deliveryMode === "SMTP" ? "验证码已发送到邮箱" : "开发验证码已生成",
    ),
  );
});
