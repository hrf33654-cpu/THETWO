...
test("verifyCodeAndCreateSession rejects expired auth code", () => {
  ...
  try {
    (config as Record<string, unknown>).authCodeTtlMinutes = 1;
    ...
    assert.throws(
      () => verifyCodeAndCreateSession(email, "654321"),
      (error: unknown) => {
        assert.equal((error as { errorCode?: string }).errorCode, "INVALID_CODE");
        return true;
      },
    );
  } finally {
    db.prepare(`DELETE FROM auth_codes WHERE email = ?`).run(email);
    (config as Record<string, unknown>).authCodeTtlMinutes = originalTtl;
  }
});
