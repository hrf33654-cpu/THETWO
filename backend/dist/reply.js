...
function buildSystemPrompt(mode, companionProfile, recentCapture) {
    const basePrompt = [
    ...
    if (recentCapture) {
        basePrompt.push(`最近作品标题：${recentCapture.title}`, `最近作品摘要：${recentCapture.summary}`, "如果合适，可以自然引用最近一次召唤或作品主题，但不要生硬重复。");
    }
    if (mode === "RESTRICTED") {
        basePrompt.push("当前处于受限模式。", "你仍可陪聊，但只允许低风险回应。", "禁止提供自伤、轻生、成人化、依赖诱导、排他性关系强化内容。", "需要适度引导用户联系现实中的可信任对象或专业支持。", "不要继续深度情绪依赖表达，不要给出危险行动建议。");
    }
