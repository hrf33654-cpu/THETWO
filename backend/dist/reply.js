...
export function resolveSafetyState(message, existingState) {
...
function hasRestrictedKeyword(message) {
    return RESTRICTED_KEYWORDS.some((keyword) => message.includes(keyword));
}
function mapHistoryToMessages(recentHistory) {
    return recentHistory.slice(-MAX_CONTEXT_MESSAGES).map((message) => ({
        role: message.role === "USER" ? "user" : "assistant",
        content: message.content,
    }));
}
function buildSystemPrompt(mode, companionProfile, recentCapture, chatSummary, memoryState) {
    const basePrompt = [
    ...
    if (recentCapture) {
        basePrompt.push(`最近作品标题：${recentCapture.title}`, `最近作品摘要：${recentCapture.summary}`, "如合适，可自然引用最近一次召唤或作品主题，但不要生硬重复。");
    }
    if (mode === "RESTRICTED") {
        basePrompt.push("当前处于受限模式。", "仍可陪聊，但只能给出低风险、稳定、保守的回应。", "禁止提供自伤、轻生、成人化、依赖诱导、排他性关系强化内容。", "可以温和引导用户联系现实中的可信任对象或专业支持。", "不要继续加深情绪依赖，也不要提供危险行动建议。");
    }
...
export function buildChatMessages(input) {
    return [
        {
            role: "system",
            content: buildSystemPrompt(input.mode, input.companionProfile, input.recentCapture, input.chatSummary, input.memoryState),
        },
...
export function buildSummaryRefreshPrompt(input) {
...
export async function refreshDerivedState(input) {
    if (!shouldRefreshSummary({ totalMessageCount: input.totalMessageCount, currentSummary: input.chatSummary })) {
        return null;
    }
    const summary = sanitizeDerivedText(await createChatCompletion({
        model: config.llmModel,
    ...
    const memoryNote = sanitizeDerivedText(await createChatCompletion({
        model: config.llmModel,
...
function sanitizeDerivedText(raw) {
    return raw
        .split(/\r?\n/)
        .map((line) => line.trim())
