...
async function authenticateAndSend(socket, reader, input) {
...
function secureSocketWithTimeout(socket) {
    socket.setTimeout(config.smtpTimeoutMs, () => {
        socket.destroy(new Error("SMTP_TIMEOUT"));
    });
}
function awaitSocketReady(socket, event) {
    return new Promise((resolve, reject) => {
...
async function expectCode(response, expectedCodes) {
